...
class Lager:
            ...
            if produkt_id in self.kampanjer:
                ...
                for _, kampanj in self.kampanjer[produkt_id].items():
                    ...
                    if kampanj_start_datum <= current_date <= kampanj_slut_datum:
                        if best_kampanj is None or kampanj.kampanj_pris < best_kampanj.kampanj_pris:
                            best_kampanj = kampanj
                if best_kampanj is not None:
                    return produkt_namn, best_kampanj.kampanj_pris, best_kampanj.kampanj_start_datum, best_kampanj.kampanj_slut_datum
    ...
    def uppdatera_kampanj(self, produkt_id, kampanj_namn, nytt_pris):
        if produkt_id in self.kampanjer and kampanj_namn in self.kampanjer[produkt_id]:
            ...
            print(f"Kampanjen '{kampanj_namn}' för produkten {produkt_id} har uppdaterats med nytt pris: {nytt_pris}")
        else:
            print(f"Kampanj med namnet {kampanj_namn} för produkten {produkt_id} existerar inte.")

    def ta_bort_kampanj(self, produkt_id, kampanj_namn):
        try:
            if produkt_id in self.kampanjer and kampanj_namn in self.kampanjer[produkt_id]:
                del self.kampanjer[produkt_id][kampanj_namn]
                print(f"Kampanjen '{kampanj_namn}' för produkten {produkt_id} har tagits bort")
                if not self.kampanjer[produkt_id]:
                    del self.kampanjer[produkt_id]
            else:
                print(f"Kampanjen med namnet {kampanj_namn} för produkten {produkt_id} existerar inte.")
        ...
        try:
            for produkt_id, produkt in self.produkter.items():
                ...
                if produkt_id in self.kampanjer:
                    ...
                    lager_str += f" ({antal_kampanjer} kampanj{'er' if antal_kampanjer > 1 else ''} tillgänglig{'a' if antal_kampanjer > 1 else ''})"
                    lager_str += "\n"
        except Exception as e:
        ...
        try:
            for produkt_id, kampanjer in self.kampanjer.items():
                ...
                for kampanj_namn, kampanj in kampanjer.items():
                    ...
                    lager_str += f"    Start datum: {kampanj.kampanj_start_datum.strftime('%Y-%m-%d')}\n"
                    lager_str += f"    Slut datum: {kampanj.kampanj_slut_datum.strftime('%Y-%m-%d')}\n"
                lager_str += "\n"
        except Exception as e:
    ...
    def spara_filer(self):
        try:
            produkter_data = [produkt.till_dict() for produkt in self.produkter.values()]
            kampanjer_data = {}
            for produkt_id, kampanjer in self.kampanjer.items():
                kampanjer_data[produkt_id] = {namn: kampanj.till_dict() for namn, kampanj in kampanjer.items()}
